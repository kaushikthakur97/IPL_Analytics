...
# MAGIC %md
# MAGIC 
# MAGIC ### Read Season_10 data from bronze_layer

# COMMAND ----------

df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=10/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2017-05-16" THEN "57"
                  WHEN match IS NULL AND match_date = "2017-05-17" THEN "58"
                  WHEN match IS NULL AND match_date = "2017-05-19" THEN "59"
                  WHEN match IS NULL AND match_date = "2017-05-21" THEN "60"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
