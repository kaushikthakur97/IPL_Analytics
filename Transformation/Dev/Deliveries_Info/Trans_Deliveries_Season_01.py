...
df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=1/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2008-05-30" THEN "57"
                  WHEN match IS NULL AND match_date = "2008-05-31" THEN "58"
                  WHEN match IS NULL AND match_date = "2008-06-01" THEN "59"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
