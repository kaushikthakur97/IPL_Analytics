...
df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=2/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2009-05-22" THEN "57"
                  WHEN match IS NULL AND match_date = "2009-05-23" THEN "58"
                  WHEN match IS NULL AND match_date = "2009-05-24" THEN "59"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
...
# MAGIC FROM df_1
# MAGIC WHERE season = "xxxxxxxxxx" OR match = "xxxxxxxxxx" OR match_date = "xxxxxxxxxx"
