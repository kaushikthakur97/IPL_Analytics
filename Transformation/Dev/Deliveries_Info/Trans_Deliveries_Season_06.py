...
# MAGIC %md
# MAGIC 
# MAGIC ### Read Season_06 data from bronze_layer

# COMMAND ----------

df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=6/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2013-05-21" THEN "73"
                  WHEN match IS NULL AND match_date = "2013-05-22" THEN "74"
                  WHEN match IS NULL AND match_date = "2013-05-24" THEN "75"
                  WHEN match IS NULL AND match_date = "2013-05-26" THEN "76"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
