...
df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=7/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2014-05-27" THEN "57"
                  WHEN match IS NULL AND match_date = "2014-05-28" THEN "58"
                  WHEN match IS NULL AND match_date = "2014-05-30" THEN "59"
                  WHEN match IS NULL AND match_date = "2014-06-01" THEN "60"
                  ELSE match
              END AS match_no,
              CASE
                  WHEN match_date = "2014-05-27" THEN "2014-05-28"
                  ELSE match_date
...
df_3 = sql("""SELECT CAST(season AS INT),
              CAST(match_date_new AS DATE) AS match_date,
