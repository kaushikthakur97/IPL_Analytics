...
# MAGIC %md
# MAGIC 
# MAGIC ### Read Season_11 data from bronze_layer

# COMMAND ----------

df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=11/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2018-05-22" THEN "57"
                  WHEN match IS NULL AND match_date = "2018-05-23" THEN "58"
                  WHEN match IS NULL AND match_date = "2018-05-25" THEN "59"
                  WHEN match IS NULL AND match_date = "2018-05-27" THEN "60"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
