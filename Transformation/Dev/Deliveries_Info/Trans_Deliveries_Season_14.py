...
df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=14/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2021-10-10" THEN "57"
                  WHEN match IS NULL AND match_date = "2021-10-11" THEN "58"
                  WHEN match IS NULL AND match_date = "2021-10-13" THEN "59"
                  WHEN match IS NULL AND match_date = "2021-10-15" THEN "60"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
