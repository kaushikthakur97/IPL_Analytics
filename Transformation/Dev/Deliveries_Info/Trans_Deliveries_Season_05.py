...
df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=5/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2012-05-22" THEN "72"
                  WHEN match IS NULL AND match_date = "2012-05-23" THEN "73"
                  WHEN match IS NULL AND match_date = "2012-05-25" THEN "74"
                  WHEN match IS NULL AND match_date = "2012-05-27" THEN "75"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
