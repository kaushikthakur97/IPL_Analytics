...
# MAGIC %md
# MAGIC 
# MAGIC ### Read Season_13 data from bronze_layer

# COMMAND ----------

df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=13/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2020-11-05" THEN "57"
                  WHEN match IS NULL AND match_date = "2020-11-06" THEN "58"
                  WHEN match IS NULL AND match_date = "2020-11-08" THEN "59"
                  WHEN match IS NULL AND match_date = "2020-11-10" THEN "60"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
