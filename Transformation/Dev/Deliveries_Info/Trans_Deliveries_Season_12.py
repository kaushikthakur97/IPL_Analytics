...
# MAGIC %md
# MAGIC 
# MAGIC ### Read Season_12 data from bronze_layer

# COMMAND ----------

df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=12/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2019-05-07" THEN "57"
                  WHEN match IS NULL AND match_date = "2019-05-08" THEN "58"
                  WHEN match IS NULL AND match_date = "2019-05-10" THEN "59"
                  WHEN match IS NULL AND match_date = "2019-05-12" THEN "60"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
