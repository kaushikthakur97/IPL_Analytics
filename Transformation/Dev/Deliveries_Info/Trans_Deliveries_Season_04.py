...
# MAGIC %md
# MAGIC 
# MAGIC ### Read Season_04 data from bronze_layer

# COMMAND ----------

df_1 = spark.read.format("delta").load("/FileStore/tables/bronze_layer/deliveries_info_data/season=4/")
...
df_2 = sql("""SELECT *, 
              CASE 
                  WHEN match IS NULL AND match_date = "2011-05-24" THEN "71"
                  WHEN match IS NULL AND match_date = "2011-05-25" THEN "72"
                  WHEN match IS NULL AND match_date = "2011-05-27" THEN "73"
                  WHEN match IS NULL AND match_date = "2011-05-28" THEN "74"
                  ELSE match
              END AS match_no
              FROM df_1
           """)
